...
module bar_engine #(
    ...
    output reg  [DATA_WIDTH -1 : 0] rdCpld_data     ,
...
//*******************DEFINE Variables************************************************/
    ...
    wire [AXIDATA_WIDTH+ADDR_VALID_WIDTH -1 : 0] wrreq_r_fifo_data;

    reg [AXIDATA_WIDTH+ADDR_VALID_WIDTH -1 : 0] rdreq_w_fifo_data;
    wire [AXIDATA_WIDTH+ADDR_VALID_WIDTH -1 : 0] rdreq_r_fifo_data;
...
//*******************PROGRAM AREA****************************************************/
    ...
    always @(posedge i_rx_pkt_clk) begin
        ...
        //写寄存器请求应当只有四个字节的数据
            ...
            wrreq_w_fifo_en <= 1'b1;
        else
            wrreq_w_fifo_en <= 1'b0; 
    end
    ...
    always @(posedge i_rx_pkt_clk) begin
        ...
        //读寄存器请求应当没有数据
        else if(rdReq_valid == 1'b1 && rdReq_dwlen == 10'd1 )
    ...
    always @(posedge i_rx_pkt_clk) begin 
        ...
        else if (rdReq_valid == 1'b1 && rdReq_dwlen == 10'd1) begin
    ...
    always @(posedge i_axi_clk) begin
        if(i_axi_rst)begin
            ...
            rdCpld_dwLen     <= 1'b1;
            rdCpld_lowaddr   <= rdreq_r_fifo_data[0 +: 7];
            rdCpld_bytecnt   <= 'd4;
            rdCpld_status    <= 'd0;//sunccess
            rdCpld_data      <= {{(DATA_WIDTH - AXIDATA_WIDTH){1'b0}},maxi_lite_rdata} ;

        end
        else begin
            rdCpld_valid     <= 'd0              ;
            rdCpld_cplid     <= rdCpld_cplid     ;
            rdCpld_reqid     <= rdCpld_reqid     ;
            rdCpld_tag       <= rdCpld_tag       ;
            rdCpld_attr      <= rdCpld_attr      ;
            rdCpld_TC        <= rdCpld_TC        ;
            rdCpld_at        <= rdCpld_at        ;
            rdCpld_dwLen     <= rdCpld_dwLen     ;
            rdCpld_lowaddr   <= rdCpld_lowaddr   ;
            rdCpld_bytecnt   <= rdCpld_bytecnt   ;
            rdCpld_status    <= rdCpld_status    ;
            rdCpld_data      <=   rdCpld_data    ;
        end
    end
