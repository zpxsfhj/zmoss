...
//*******************PROGRAM AREA****************************************************/
    ...
    //将rx_in_eof 相关信号延拍，为了生成输出数据的结束标志
    always @(posedge i_clk) begin
        if(i_rst)begin
        ...
        if(DATA_WIDTH == 64)begin:DATA_WIDTH0
            always @(posedge i_clk) begin
                ...
                //如果为64位地址
                    ...
                    tlp_address <= {rx_in_data[39:32],rx_in_data[47:40], rx_in_data[55:48],rx_in_data[63:56],
                                    rx_in_data[7:0],rx_in_data[15:8], rx_in_data[23:16],rx_in_data[31:26], 2'b00};
                //如果为32位地址
                else if(rx_in_valid_r[0] & rx_in_sof_r[0] & ~tlp_fmt[0])
                    tlp_address <= {32'd0,rx_in_data[7:0],rx_in_data[15:8], rx_in_data[23:16],rx_in_data[31:26],2'b00};
                else
                    tlp_address <= tlp_address;
            end
            //如果没有数据，则将数据有效信号拉高一个时钟周期，将包头信息传递下去。
            //如果有数据，需要将输入数据有效信号的前两个时钟周期去掉(包头)
            ...
            //如果是四个双字的包头，不需要数据对齐，如果是三个双字，需要移位32进行数据对齐
