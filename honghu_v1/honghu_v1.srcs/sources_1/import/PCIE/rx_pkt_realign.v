...
module rx_pkt_realign #(
    ...
    output  wire 	[DATA_WIDTH -1:0] rx_out_data	     
...
//*******************DEFINE Variables************************************************/
    ...
    reg  [DATA_WIDTH -1:0]     rx_data_swap;
    ...
    reg [DATA_WIDTH -1:0] rx_out_data_pre ;
//*******************INSTANCE AREA***************************************************/
...
//*******************PROGRAM AREA****************************************************/
    ...
    generate
        for(i = 0;i < DATA_WIDTH_BYTE/4 ; i=i+1)begin:swap_loop0
            for (j = 0 ; j < 4 ; j=j+1)begin:swap_loop1
                always @(i_clk) begin
                    if(i_rst)
                        rx_data_swap[8*(i*4 + j) +:8] <= 'd0;
                    else
                        rx_data_swap[8*(i*4 + j) +:8] <= rx_out_data_pre[ 8*(i*4 +3 - j) +:8];
                end
            end
        end
    endgenerate
    assign rx_out_data = rx_data_swap ;
    //对输入数据进行移位存储，用于字节对齐
    always @(posedge i_clk) begin
        if(i_rst)
            rx_data_2r <= 'd0;
        else
            rx_data_2r <= {rx_in_data,rx_data_2r[DATA_WIDTH*2 - 1 : DATA_WIDTH]};
    end
    ...
    //将rx_sof_index 缓存两拍，与数据输出有效信号对齐，因为输出数据也缓存了两拍
    always @(posedge i_clk) begin
        if(i_rst)begin
            rx_sof_index_r  <= 'd0;
            rx_sof_index_2r <= 'd0;
        ...
        if(DATA_WIDTH_BYTE > 8)
            always @(*) begin
                case (rx_sof_index_2r)
                    4'd0: rx_out_data_pre = rx_data_2r[DATA_WIDTH - 1    : 0];
                    4'd4: rx_out_data_pre = rx_data_2r[DATA_WIDTH - 1 +4*8 : +4*8];
                    4'd8: rx_out_data_pre = rx_data_2r[DATA_WIDTH - 1 +8*8 : +8*8];
                    4'd12: rx_out_data_pre = rx_data_2r[DATA_WIDTH - 1 +12*8 : +12*8];
                    default: rx_out_data_pre = rx_data_2r[DATA_WIDTH - 1 : 0];
                endcase
            end
        else
            always @(*) begin
